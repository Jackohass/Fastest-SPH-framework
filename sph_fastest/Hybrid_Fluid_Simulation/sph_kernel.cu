...
//stf Calculate density of a cell 计算density 一个cell
...
//stf Calculate density 计算density
__global__ //__launch_bounds__(kDefaultNumThreadSMS, kDefulatMinBlocksSMS)
...
//stf Calculate force a cell 计算 force 一个cell
__device__
...
//stf Update speed position new processing method 更新速度位置 新的处理方式
//__global__
...
//stf Update position velocity Large solid boundary, original boundary processing method 更新位置速度  大的固体边界，原先的边界处理方式
...
//stf PCISPH-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
//stf Calculate PCISPH density error factor 计算PCISPH密度误差因子
__device__
...
//stf step1 predicted position 预测位置
__global__
...
//stf step2 Calculate predicted density 计算预测后的密度
__device__
...
//stf step3 Find the maximum density error 找最大密度误差
__global__
...
//stf step4 Calculate the force generated by pressure 计算pressure产生的力
__device__
...
//stf host calculates the total function of density host计算density的总函数
...
//stf Host calculates the total function of force host计算force的总函数
...
//stf Update position speed 更新位置速度
...
//stf PCISPH prediction verification main function PCISPH预测校验主函数----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void predictionCorrectionStepHybrid128(ParticleBufferList buff_list, int *cell_offset, int *cell_num, BlockTask *block_task, int num_block,
    ...
    while ((iteration < pcisph_min_loop) || ((densityErrorTooLarge) && (iteration < pcisph_max_loop)))
    {
        ...
        predictPositionAndVelocity(buff_list, nump);   //stf There are solid-liquid interaction steps 有固液交互步骤
...
void predictionCorrectionStepHybrid128n(ParticleBufferList buff_list_n, int *cindex, int *cell_offset, int *cell_num, BlockTask *block_task, int num_block,
    ...
    while ((iteration < pcisph_min_loop) || ((densityErrorTooLarge) && (iteration < pcisph_max_loop)))
    {
        ...
        predictPositionAndVelocity(buff_list_n, nump);   //stf There are solid-liquid interaction steps 有固液交互步骤

        //predictPositionAndVelocity(buff_list_o, nump);   //stf There are solid-liquid interaction steps 有固液交互步骤
        max_predicted_density = 1000.0f;
                                  ...
                                  , float pcisph_density_factor, unsigned int nump, int pcisph_min_loop, int pcisph_max_loop, float pcisph_max_density_error_allowed)
{
    ...
    while ((iteration < pcisph_min_loop) || ((densityErrorTooLarge) && (iteration < pcisph_max_loop)))
    {
        ...
        //      predictPositionAndVelocity(num_block, buff_list, nump, cell_start, cell_end, block_task, num_block);   //stf There are solid-liquid interaction steps 有固液交互步骤
        predictPositionAndVelocity(buff_list, nump);   //stf There are solid-liquid interaction steps 有固液交互步骤
        max_predicted_density = 1000.0f;
                                 ...
                                 float pcisph_density_factor, unsigned int nump, int pcisph_min_loop, int pcisph_max_loop, float pcisph_max_density_error_allowed, ParticleIdxRange range)
{
    ...
    while ((iteration < pcisph_min_loop) || ((densityErrorTooLarge) && (iteration < pcisph_max_loop)))
    {
        ...
        //      predictPositionAndVelocity(num_block, buff_list, nump, cell_start, cell_end, block_task, num_block);   //stf There are solid-liquid interaction steps 有固液交互步骤
        predictPositionAndVelocity(buff_list, numpp);   //stf There are solid-liquid interaction steps 有固液交互步骤
        max_predicted_density = 1000.0f;
